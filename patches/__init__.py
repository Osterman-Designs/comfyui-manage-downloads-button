...
def start():
    ...
    if args.enable_manager:
        if args.enable_manager_legacy_ui:
            ...
            try:
                from . import bridge_routes  # noqa: F401
                bridge_routes.enable()
            except Exception as e:
                import traceback
                logging.error(
                    "[ComfyUI-Manager] Model Manager bridge routes failed: "
                    f"{type(e).__name__}: {e}\n{traceback.format_exc()}"
                )
...
def should_be_disabled(fullpath:str) -> bool:
    ...
    if args.enable_manager:
        ...
        if 'comfyui-manager' in dir_name and 'comfyui-manager-bridge' not in dir_name:
            return True
